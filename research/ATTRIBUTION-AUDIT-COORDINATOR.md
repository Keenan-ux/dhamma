...
**All three items (A1, A2, A3) LANDED, PUSHED, and DEPLOYED.** Campaign fully shipped 2026-06-19 (operator authorized the redeploy; smoke green: `/api/dbcheck` 194710, research gate 401). Nothing pending. **Key metric (now corrected):** the deduped, per-claim-coded, IAA'd **buddha-vacana count** for the 299 = **17 rows / 9 distinct recollections** (was a false 0). **Pre-registered predictions** (score verbatim after each lands; a wrong prediction is a finding):
