...
CONTEXT = {
 "agganna_parallel": {
   "claim": "The self-ripening rice is not unique to Uttarakuru. DN 27 (Aggañña-sutta) gives the same formula for the cosmic golden age, and the Vessantara Jātaka gives it for the forest hermitage (sāli akaṭṭhapāko). The Uttarakuru ethnography recapitulates a wandering canonical idyll, and the commentary echoes its very wording (akaṇaṃ athusaṃ sugandhaṃ).",
   "cites": [cite("DN 27 (Aggañña)", "dn27"), cite("DN 32", "dn32"), cite("JA 547 (Vessantara)", "ja547")],
 },
 "akkhana_control": {
   "claim": "The soteriological bar is not in the canonical inopportune-birth list. AN 8.29 and DN 33 enumerate the eight akkhaṇā asamayā brahmacariyavāsāya (hell, animal, ghost, long-lived deva, border-region, wrong-view, dullness, no-Buddha-arisen); none names Uttarakuru. The long-lived-deva moment is the nearest, and the commentary extends that reasoning to assimilate the Uttarakurukas. The Uttarakuru disqualification is therefore commentarial, not a canonical akkhaṇa.",
...
DATA = {
 "meta": {
  ...
  "corrected_headline": "A tiny early-canonical seed (a named, propertyless, long-lived people, no measure, no figure) materializes into a described place inside a late-canonical paritta, then a visited place in the Apadana and Vinaya frame-narratives, then a measured place at the commentarial boundary. The ethnographic template is pan-Buddhist bedrock; the soteriological verdict is Pali-local-pending-verification. At no rung does the canon submit the geography to its own test of verified knowledge: materialization and verification are different axes, and only the first one moves. The full v2 paper is research/uttarakuru/FINDINGS-v2.md.",
  "version_note": "v1.3: recoded under the provenance-signature framework (PROVENANCE-SIGNATURE.md). Each feature now carries a per-claim signature (chronological stratum coded independently of structural layer, attribution, epistemic marking, cross-recensional reach, harmonization). The headline finding: of 26 structurally-mula rows only 6 are early-canonical; the literal-place reading is more concrete in the later/narrative registers (a gradient flagged as partly definitional, see FINDINGS-v2.md). v1.2: corrected the short-u substring (144) to the honest stem '%uttarakur%' (161), recovering the canonical four-continent cosmology (AN 3.80 / AN 10.29).",
 ...
 "reliability": {
  ...
  "note": "Three independent blind coders classified each feature's canonical warrant and H0/H1 class from the verbatim evidence. Agreement was unanimous on 15 of 16 features (Fleiss kappa 0.94, almost perfect) and on every warrant (canonical-id vs null). The census voice axis is deterministic from the source work; motif is an approximate window-based facet.",
  "signature_coders": _v2.get("coders", 3),
  "signature_iaa": {k: (str(v["unanimous"]) + "/" + str(v["of"])) for k, v in _v2["iaa"].items()},
  "signature_note": "The v2 per-claim signature (chronological stratum, attribution, epistemic marking, cross-recensional reach, harmonization) was coded by three independent blind coders; agreement per axis is reported above (12-15 of 16). The expensive axes (attribution, harmonization) carry the most coder disagreement, as expected, since they require judging speaker and reconciliation-intent beyond a marker search.",
 },
 "saturation": {
 ...
 "aggregates": {
  ...
  "cross_recension": {
   ...
   "tithanasutta_hinge": {"for": "an9.21", "status": "pali-local-unconfirmed",
     "parallels": [], "guard": "no LINKED non-Pali parallel is not 'no parallel exists'; the schema is stock Abhidharma cosmology a sutta-parallel table under-covers."},
  },
 },
}
