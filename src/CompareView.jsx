...
import { countOccurrences, kwic, frequencyByTradition, neighborsByTradition } from './analyze.js';
import PassageCard from './PassageCard.jsx';

export default function CompareView({ term, activeTraditions }) {
  const t = (term || '').trim();
  const filtered = activeTraditions
    ? SAMPLE_PASSAGES.filter((p) => activeTraditions.has(p.tradition))
    : SAMPLE_PASSAGES;
  ...
  const stats = useMemo(() => {
    ...
    const matching = filtered.filter((p) => {
      ...
      return countOccurrences(hay, t) > 0;
    });
    const totalOcc = matching.reduce(
      (s, p) => s + countOccurrences(`${p.original || ''} ${p.translation || ''}`, t),
      0
    );
    const freqByTrad = frequencyByTradition(matching, t);
    const neighbors = neighborsByTradition(matching, t, 5, 10);
    ...
    const kwicRows = matching.flatMap((p) =>
      [
        ...kwic(p.original || '', t, 7),
        ...kwic(p.translation || '', t, 7),
      ].map((row) => ({ ...row, citation: p.citation, tradition: p.tradition }))
    );
    return { matching, totalOcc, freqByTrad, neighbors, byTradition, kwicRows };
  }, [t, activeTraditions]);
  ...
  if (!stats || stats.matching.length === 0) {
    return (
      <div style={{ position: 'absolute', inset: 0, overflow: 'auto' }}>
        <div style={wrap}>
          <h2 style={h1}>Comparing <em style={{ color: 'var(--bc-accent)' }}>{t}</em></h2>
          <p style={meta}>No occurrences in the visible corpus. Try a different inflection or term.</p>
        </div>
      </div>
    );
  }

  const maxFreq = Math.max(...stats.freqByTrad.map((f) => f.count));
    ...
    <div style={{ position: 'absolute', inset: 0, overflow: 'auto' }}>
      <div style={wrap}>
        <div style={{ marginBottom: 28 }}>
          <h2 style={h1}>Comparing <em style={{ color: 'var(--bc-accent)' }}>{t}</em></h2>
          <p style={meta}>
            {stats.totalOcc} occurrences across {stats.matching.length} {stats.matching.length === 1 ? 'passage' : 'passages'} in {stats.freqByTrad.length} {stats.freqByTrad.length === 1 ? 'tradition' : 'traditions'}.
          </p>
        </div>

        <Section title="Frequency by tradition">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {stats.freqByTrad.map(({ tradition, count }) => (
        ...
        <Section title="Side by side">
          <div style={columnsGrid}>
            {Array.from(stats.byTradition.entries()).map(([trad, passages]) => (
        ...
        <Section title="Concordance" subtitle="Every occurrence centered on the term with ±7 words context.">
          <div style={{ overflowX: 'auto' }}>
            <table style={kwicTable}>
              <tbody>
                {stats.kwicRows.map((row, i) => (
                  <tr key={i}>
        ...
        <Section title="Companion words" subtitle="What appears within ±5 words of the term, by tradition. Stopwords filtered.">
          <div style={columnsGrid}>
            {Array.from(stats.neighbors.entries()).map(([trad, words]) => (
              <div key={trad} style={column}>
                <h3 style={columnHeading}>{trad}</h3>
                {words.length === 0 ? (
