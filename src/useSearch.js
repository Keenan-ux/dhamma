...
export default function useSearch({ q, mode, field, limit, nosnippet, pitaka, layer, translator, tag }) {
  ...
  const snapshotRef = useRef({ q: '', mode: '', field: '', nosnippet: false, limit: 0, pitaka: '', layer: '', translator: '', tag: '' });
  ...
  useEffect(() => {
    ...
    const snapshot = { q, mode, field, nosnippet: !!nosnippet, limit, pitaka: pitaka || '', layer: layer || '', translator: translator || '', tag: tag || '' };
    snapshotRef.current = snapshot;

    setLoading(true);
    setLoadingMore(false);
    const t = setTimeout(() => {
      searchApi({ q, mode, field, limit, offset: 0, nosnippet, pitaka, layer, translator, tag, signal: ctrl.signal })
        .then((r) => {
  ...
  }, [q, mode, field, nosnippet, limit, pitaka, layer, translator, tag]);
  ...
  const loadMore = useCallback(() => {
    ...
    searchApi({
      ...
      translator: snap.translator || undefined,
      ...
      .then((r) => {
        ...
        if (now.q !== snap.q || now.mode !== snap.mode || now.field !== snap.field
            ...
            || now.translator !== snap.translator || now.tag !== snap.tag) {
          return;
        }
        setState((s) => ({
          ...s,
